...
namespace armor_detector {
    ...
    void CameraProvider::init() {
        ...
        camera_info_.camera_matrix = cv::Matx33d(
            cm_data[0], cm_data[1], cm_data[2], cm_data[3], cm_data[4], cm_data[5], cm_data[6], cm_data[7], cm_data[8]);

        // 读取 distortion_coefficients (1x5)
        auto dc_data = config["distortion_coefficients"]["data"].as<std::vector<double>>();
        camera_info_.distortion_coefficients =
            cv::Vec<double, 5>(dc_data[0], dc_data[1], dc_data[2], dc_data[3], dc_data[4]);
        ...
        camera_info_.rectification_matrix = cv::Matx33d(
            rm_data[0], rm_data[1], rm_data[2], rm_data[3], rm_data[4], rm_data[5], rm_data[6], rm_data[7], rm_data[8]);

        // 读取 projection_matrix (3x4)
        auto pm_data = config["projection_matrix"]["data"].as<std::vector<double>>();
        camera_info_.projection_matrix = cv::Matx34d(pm_data[0],
                                                     pm_data[1],
                                                     pm_data[2],
                                                     pm_data[3],
                                                     pm_data[4],
