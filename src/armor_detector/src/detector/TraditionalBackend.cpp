...
namespace armor_detector {
    ...
    TraditionalArmorDetectorBackend::TraditionalArmorDetectorBackend(const Params &params) :
        params_(params), light_detector_(params.light), armor_detector_(params.armor),
        number_classifier_(params.number) {
    }

    cv::Mat TraditionalArmorDetectorBackend::preprocess(const cv::Mat &img_bgr) {
        // COLOR 阈值
        std::vector<cv::Mat> bgr;
        cv::split(img_bgr, bgr);
        cv::Mat color_ch = (params_.preprocess.target_color == LightBarColor::BLUE) ? bgr[0] : bgr[2];
        cv::Mat target_color_dim_thre;
        cv::threshold(color_ch, target_color_dim_thre, params_.preprocess.color_threshold, 255, cv::THRESH_BINARY);

        // GRAY 阈值
        cv::Mat gray;
        cv::cvtColor(img_bgr, gray, cv::COLOR_BGR2GRAY);
        cv::Mat gray_thre;
        cv::threshold(gray, gray_thre, params_.preprocess.gray_threshold, 255, cv::THRESH_BINARY);
    ...
    DetectionResult TraditionalArmorDetectorBackend::detect(const cv::Mat &img_bgr) {
        DetectionResult result;
        result.debug.backend = DetectionBackend::TRADITIONAL;

        using Clock = std::chrono::steady_clock;
        using ms = std::chrono::duration<double, std::milli>;

        auto t0 = Clock::now();
        cv::Mat img_thre = preprocess(img_bgr);
        auto t1 = Clock::now();

        auto lights = light_detector_.findLights(img_thre, img_bgr);
        auto t2 = Clock::now();

        auto candidates = armor_detector_.match(lights);
        auto t3 = Clock::now();

        auto classified = number_classifier_.classify(candidates, img_bgr);
        auto t4 = Clock::now();
        ...
        result.debug.traditional.armor_match = armor_detector_.getArmorMatchDebugData();
        result.debug.traditional.classification = number_classifier_.getClassificationDebugData();

        result.debug.timings.push_back({"preprocess", std::chrono::duration_cast<ms>(t1 - t0).count()});
        result.debug.timings.push_back({"lights", std::chrono::duration_cast<ms>(t2 - t1).count()});
        result.debug.timings.push_back({"armor_match", std::chrono::duration_cast<ms>(t3 - t2).count()});
        result.debug.timings.push_back({"number_classify", std::chrono::duration_cast<ms>(t4 - t3).count()});
